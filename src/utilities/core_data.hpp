...
namespace jadeite
{
	/* 	Global engine state singleton.
		Holds cross-system configuration that must be accessible everywhere without passing references: 
		window dimensions, the pixel-to-meter conversion factor (pixels per Box2D unit), 
		the scaled render resolution, and physics solver settings (iterations, gravity, enable/pause).*/
	class CoreData
	{
	public:
