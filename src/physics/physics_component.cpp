...
namespace jadeite
{
	...
	/* Initialize the physics components with the physics world and the width and height of the physics world */	
	...
	/* Returns if the rigidBody has the sensor flag */
	...
	/* Fires a ray from point1 to point2 and reports what it hits */
	...
	/* 	Swept box collision query — it sweeps an oriented box from one position to another and reports the first fixture hit. 
		Unlike CastRay (zero-width segment), a box trace has volume, so it catches edges and corners a ray would miss. */
	...
	/* Sets the category bits for this fixture (what I am). A fixture is tested against another's maskBits to determine whether a contact is generated. */
	...
	/* Sets the filter category back to the initial category */
	void PhysicsComponent::SetFilterCategory( )
	{
		SetFilterCategory( m_InitialAttribs.filterCategory );
	}

	/* Sets the mask bits for this fixture (what I collide with). A contact is generated only if BOTH fixtures' category/mask */
	...
	/* Sets the filter mask back to the initial attributes */
	void PhysicsComponent::SetFilterMask( )
	{
		SetFilterMask( m_InitialAttribs.filterMask );
	}
	
	/*	 Sets the group index for same-category filtering.
		> 0: always collides with fixtures in the same group
		< 0: never collides with fixtures in the same group
		  0: falls through to the normal category/mask test	*/
	...
	/* Sets the group index back to the initial attributes */
